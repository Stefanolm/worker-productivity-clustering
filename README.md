...
1. **Preprocesamiento:** imputación KNN de valores faltantes, transformación logarítmica de variables con alta dispersión (productividad, días trabajados, ingreso total) y estandarización Z.
2. **Selección del número de clusters:** método del codo (evolución de la inercia intra-cluster) evaluando de 1 a 15 clusters.
3. **Modelo final:** K-Means con **k=4**, `n\\\\\\\_init=20`, `random\\\\\\\_state=123`.
4. **Validación:** comparación de K-Means contra Gaussian Mixture Model (GMM) usando Silhouette Score, Davies-Bouldin Score y Calinski-Harabasz Score.
5. **Visualización:** proyección de los clusters en 3 componentes principales (PCA) para inspección visual de la separación entre grupos.
